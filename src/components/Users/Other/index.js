...
const Other = ({users,downloadFile,updateUserStatus,deleteUser,fetchData}) => {
    ...
    const exportToCSV = () => {
        ...
        // CSV header
        const headers = ["ID", "Email", "First Name","Last Name","Contact No","WhatsApp","Gender","Course Type", "Profile Image","Resume"];

        // Convert data rows into CSV format
        const rows = users.map(user => [user.id, user.email, user.first_name, user.last_name,user.contact_no,user.whatsapp_no,user.gender,user.course_type,user.profile_image,user.resume]);
